...
class prediction():
    ...
    def load_raw(self):
        with open(r'linear_model.pkl','rb') as model_file: 
            self.model = pickle.load(model_file) ##artifacts
    
        with open(r'columns_names.json','r') as col_file: 
            self.column_names = json.load(col_file)
    
        with open(r'encoded_data.json','r') as encode_file:
    ...
    def predict_price(self,data):
        ...
        drive_wheels = self.data["html_drive_wheels"]
